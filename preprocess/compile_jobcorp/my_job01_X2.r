...
# Nonresidential slot
X$nonres <- f_b9d      # 1 if designated for a nonresidential slot

# b) Education
#---------------
                                          
# Diploma
X$educ_geddiploma       <- f_c2c # dummy for category 2 
X$educ_hsdiploma        <- f_c2d # dummy for category 3 
                                     
# Highest grade completed and its squared : [0,20], 98 or 99 if not answered 
X$hgrd  <- f_c1          # What is the highest grade you have already completed?
...
weekly_earn <- 
         data.frame(f_d351,     #weekly earnings on job 1             
                    f_d352,     #weekly earnings on job 2
                    f_d353,     #weekly earnings on job 3                  
                    f_d354,    #weekly earnings on job 4
                    f_d35a,     #weekly earnings on job 5
                    f_d35b,     #weekly earnings on job 6
                    f_d35c,     #weekly earnings on job 7
                    f_d35d,     #weekly earnings on job 8   (only NAs)
                    f_d35e,     #weekly earnings on job 9   (only NAs)
                    f_d35h)     #weekly earnings on job 10  (only NAs)
...
X$g2<-f_g3  #recvd afdc every month since ref date
...
X$g5<-f_g14  #got public assist. every month
X$g5[X$g5>=8]<- NA # Replace the 8-9 answers by NA 

X$g7<-f_g44  #recvd food stamps since ref date
...
X$haschldY0 <- f_f25  # 0 if no child, 1 if one child or more

# Marital status on randomization date
X$marstat0 <- f_f23   #1=married, 2=separated, 3= divorced, 4=widowed, 5=living together unmarried, 6=never married, and not living together unmarried
X$marstat_married        <- indicator(f_f23==1) # dummy for category 1
X$marstat_separated      <- indicator(f_f23==2) # dummy for category 2  
X$marstat_divorced       <- indicator(f_f23==0) # dummy for category 3
X$marstat_widowed        <- indicator(f_f23==4) # dummy for category 4
X$marstat_livetogunm     <- indicator(f_f23==5) # dummy for category 5
X$marstat_notlivetogunm  <- indicator(f_f23==6) # dummy for category 6
...
X$h1_excel    <- indicator(f_h1==1) # dummy for category 1
X$h1_good     <- indicator(f_h1==2) # dummy for category 2
X$h1_fair     <- indicator(f_h1==3) # dummy for category 3
X$h1_poor     <- indicator(f_h1==4) # dummy for category 4
...
X$h10<-f_h10  # how often used marijuana in last year 
...
X$h25<-f_h20  # how often used lsd/peyote/psilocybin/hallucinogenic drugs in last year
...
X$h29<-f_h23  # ever used other illegal drugs
