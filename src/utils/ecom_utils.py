...
def check_high_dimensional(X, T, *, threshold, featurizer=None, discrete_treatment=False, msg=""):
    # Check if models is sparse enough for this models
...
def fit_with_groups(model, X, y, groups=None, **kwargs):
    """
    Fit a models while correctly handling grouping if necessary.
    This enables us to perform an inner-loop cross-validation of a models
    ...
    groups : array-like, optional
        ...
        cross-validation
    kwargs : dict
        Any other named arguments to pass to the models's fit
    """

    if groups is not None:
        # assume that we should perform nested cross-validation if and only if
        # the models has a 'cv' attribute; this is a somewhat brittle assumption...
        if hasattr(model, 'cv'):
            old_cv = model.cv
...
class WeightedModelWrapper:
    ...
    sample_type : string, optional (default=`weighted`)
        Method for adding weights to the models. `weighted` for linear regression models
    ...
    def __init__(self, model_instance, sample_type="weighted"):
        ...
        if sample_type == "weighted":
            ...
            warnings.warn("The models provided does not support sample weights. "
    ...
    def fit(self, X, y, sample_weight=None):
        """Fit underlying models instance with weighted inputs.
        Parameters
        ----------
        ...
        return self.model_instance.fit(X, )
...
class SeparateModel:
    ...
    a separate models for each subsample. At predict time, it
    uses the last feature to choose which models to use
    to predict.
    """
