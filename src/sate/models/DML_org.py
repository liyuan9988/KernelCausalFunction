    ...
    def _fit_cov_matrix(self, resT, resY, models):
        ...
        for kappa in np.arange(n_periods):
            ...
            res_epsilon = (resY[period]
                           - np.sum(
                        [models[tau].predict(resT[period][n_periods - 1 - tau].reshape(-1, self._n_treatments))
...
def evaluate_DML_org(data_config: Dict[str, Any], model_param: Dict[str, Any],
    ...
    model = DynamicPanelDML()
    n_units = train_data.backdoor_1st.shape[0]
    n_periods = 2
    n_treatments = train_data.treatment_1st.shape[1]
    n_x = train_data.backdoor_1st.shape[1]
    Y = np.zeros(n_units * n_periods)
    T = np.zeros((n_units * n_periods, n_treatments))
    X = np.zeros((n_units * n_periods, n_x))
    groups = np.zeros(n_units * n_periods)
    for t in range(n_units * n_periods):
        period = t % n_periods
        if period == 0:
            X[t] = train_data.backdoor_1st[t // 2]
            T[t] = train_data.treatment_1st[t // 2]
            Y[t] = 0.0
        else:
            X[t] = train_data.backdoor_2nd[t // 2]
            T[t] = train_data.treatment_2nd[t // 2]
            Y[t] = train_data.outcome[t // 2]
        groups[t] = t // n_periods

    if T.shape[1] == 1:
        T = T[:,0]

    model.fit(Y, T, X, groups)
    pred = model.param[-1] * test_data.treatment_1st + model.param[-2] * test_data.treatment_2nd
