...
def generate_test_synthetic_linear() -> SATETestDataSet:
    """
    Returns
    -------
    ...
    structural = -0.6 * np.linalg.norm(theta)**2 * treatment_1st
    structural += 1.2 * treatment_2nd
...
def generate_train_synthetic_linear(data_size: int,
    ...
    backdoor_cov += np.diag([0.5] * (backdoor_dim - 1), -1)
    backdoor_1st = 0.5 * rng.multivariate_normal(np.zeros(backdoor_dim), backdoor_cov,
                                                 size=data_size)  # shape of (data_size, backdoor_dim)

    theta = np.array([1.0 / ((i + 1) ** 2) for i in range(backdoor_dim)])[:, np.newaxis]
    treatment_1st = norm.cdf(backdoor_1st.dot(theta) * 3) + 0.75 * rng.standard_normal(size=(data_size, 1))
    backdoor_2nd = 0.5 * (backdoor_1st - treatment_1st * (theta.T))
    backdoor_2nd += 0.5 * rng.multivariate_normal(np.zeros(backdoor_dim), backdoor_cov,
                                                  size=data_size)
    treatment_2nd = norm.cdf(3 * backdoor_2nd.dot(theta) - treatment_1st)
